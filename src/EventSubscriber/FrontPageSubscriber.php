...
class FrontPageSubscriber implements EventSubscriberInterface {
  ...
  public function initData(GetResponseEvent $event) {
    ...
    if ($front_page) {
      ...
      // Get system default front page internal node url.
      $system_front_page = \Drupal::configFactory()->get('system.site')->get('page.front');

      // Get alias from url
      $system_front_page = \Drupal::service('path.alias_manager')->getAliasByPath($system_front_page);

      $current_language = \Drupal::languageManager()->getCurrentLanguage();
      $url = Url::fromUserInput($front_page, ['language' => $current_language]);

      // compare system default front page url with overridden front url
      if ($system_front_page != $url->toString()) {
